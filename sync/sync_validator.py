...
class SyncValidator(SyncBase):
    ...
    def retrieve_data(self):
        ...
        cursor_destination = self.destination_conn.cursor(cursor_factory=extras.RealDictCursor)
        sql_query = sql.SQL(get_studies_by_date).format(schema=sql.Identifier(self.schema_name))
        cursor_destination.execute(sql_query, {"start_date": start_date, "end_date": end_date})
