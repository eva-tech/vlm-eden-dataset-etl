-- Query to get all DICOM files and reports from all studies in pacs_studies
-- Following the pattern with CTE for all studies

WITH all_studies AS (
  -- Select all studies from pacs_studies (29 rows)
  SELECT id
  FROM pacs_studies
  WHERE deleted = FALSE
)
SELECT 
  ...
  CONCAT('https://files.evacenter.com/media/', pif.original_file) AS file_url,
